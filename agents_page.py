...
from api import add_agent, get_agent_list, get_agent_parameters, update_model, delete_model
...
def show_agents():
    """Function to show the agents screen."""
    st.header("Agents")
        ...
        if "edit_agent" in st.session_state and st.session_state["edit_agent"] == agent:
            ...
            with col1:
                if st.button("💾 Save Agent"):
                    update_model(available_agents.index(st.session_state["edit_agent"]), agent_parameters)
                    st.session_state["edit_agent"] = ""
                    st.rerun()
            with col2:
                if st.button("🗑️ Delete Agent"):
                    delete_model(available_agents.index(st.session_state["edit_agent"]))
                    st.session_state["edit_agent"] = ""
                    st.rerun()
            with col3:
                if st.button("❌ Cancel"):
